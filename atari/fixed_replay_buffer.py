...
class FixedReplayBuffer:
  ...
  def __init__(self, data_dir, replay_suffix, *args, **kwargs):
    ...
    self._game = os.path.basename(data_dir) 
    filename = os.path.join(self._data_dir, f'{self._game}NoFrameskip-v4_{self._replay_suffix}.npz')
    
    self.load_single_buffer(filename)


  def load_single_buffer(self, buffer_filename):

    try:
      with open(buffer_filename, 'rb') as f:
        replay_data = np.load(f)
        self._observations = replay_data['obs']
        self._actions = replay_data['taken actions']
        self._rewards = replay_data['rewards']
        self._terminals = replay_data['episode_starts']
        self._loaded_buffers = True
        # print(f"Loaded buffer {buffer_filename} with {len(self._observations)} transitions.")

    except FileNotFoundError:
      # print(f"Unable to load replay buffer {buffer_filename}")
      self._loaded_buffers = False


  def sample_transition_batch(self, batch_size=32, indices=None):
    """Amostra um lote de transições do buffer."""
    if not self._loaded_buffers:
      # Retorna dados vazios se o buffer não foi carregado para evitar erros
      return ([], [], [], [], [], [], [], [])

    if indices is None:
      indices = np.random.randint(len(self._observations) - 1, size=batch_size)
    else:
    ...
    return (
        ...
        self._observations[indices + 1], # next_observation
        self._actions[indices + 1], # Simula next_action
        self._rewards[indices + 1], # Simula next_reward
